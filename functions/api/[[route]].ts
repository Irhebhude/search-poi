...
async function route(head: string, rest: string[], request: Request, env: Env, url: URL): Promise<Response> {
  ...
  // FIXED: API KEYS ROUTE - works with old and new schema
  if (head === "keys") {
    ...
    if (request.method === "GET" && rest.length === 0) {
      try {
        const result = await env.DB.prepare(
          `SELECT id, COALESCE(key_hash, key) as key, 0 as requests_count, null as period, created_at, COALESCE(is_active, 1) as is_active FROM api_keys WHERE user_id =? OR userId =? ORDER BY created_at DESC`
        ).bind(session.userId, session.userId).all();

        return json({ keys: result.results || [], quota: 1000 });
      } catch(e) {
        return json({ error: "DB Error: " + (e as Error).message }, 500);
      }
    }
    ...
    if (request.method === "POST" && rest[0] === "generate") {
      try {
        ...
        // Deactivate old keys - works for both column names
        await env.DB.prepare(`UPDATE api_keys SET is_active = 0 WHERE user_id =? OR userId =?`).bind(session.userId, session.userId).run();

        // Try new schema first
        try {
          await env.DB.prepare(
            `INSERT INTO api_keys (id, user_id, name, key_hash, created_at, is_active) VALUES (?,?,?,?,?, 1)`
          ).bind(id, session.userId, "Default", keyHash, now).run();
        } catch {
          // Fallback to old schema
          await env.DB.prepare(
            `INSERT INTO api_keys (id, user_id, name, key, created_at, is_active) VALUES (?,?,?,?,?, 1)`
          ).bind(id, session.userId, "Default", keyHash, now).run();
        }
